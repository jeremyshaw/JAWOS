...
typedef struct {
	char *dsp_str;	// addr of string to print
	que_t dsp_wait_que;	//requesting process
	int port;	// set to TTY0/1/2/ - 0x2f8 COM2, 0x3e8 COM3, 0x2e8 COM4
	char *kb_str;	// string ptr to get terminal input (KB)
	que_t kb_wait_que;	// terminal input strings
	que_t echo;	// input is echoed back to terminal
} tty_t;
